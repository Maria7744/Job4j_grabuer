...
public class UploadCreat {
    private List<String> temp = new ArrayList<>();
        private static final Logger LOG = LogManager.getLogger(UploadTo.class.getName());

        UploadCreat() {

    }

        UploadCreat(Properties config) {
        ...
        try (PreparedStatement pst = connection.prepareStatement("insert into sqltable (names, alltext, links) "
            ...
            for (var e : exm) {
                ...
                pst.setString(2, e.text);
                pst.setString(3, e.link);
                pst.setString(4, e.text);
